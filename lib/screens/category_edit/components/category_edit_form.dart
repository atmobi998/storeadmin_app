...
class _CategoryEditFormState extends State<CategoryEditForm> {
  ...
  String catslug = (admcurcat.isNotEmpty)? admcurcat.first.catslug : ''; 
  String catdesc = (admcurcat.isNotEmpty)? admcurcat.first.catdesc : ''; 
  String catsort = (admcurcat.isNotEmpty)? "${admcurcat.first.catsort}" : "" ;
  ...
  categorySaveChanges() {
    ...
    http.post(categoryeditEndPoint, body: postdata, headers: posthdr ).then((result) {
        if (result.statusCode == 200) {
          ...
          if (resary['status'] > 0) {
              setState(() {
                ...
                curcatactive = (admcurcat.first.catactive > 0);
              });
